...
public class AwsTopicFluentBuilder(ITopicProviderFactory topicProviderFactory, ILogger<AwsTopicFluentBuilder> logger) : IAwsTopicFluentBuilder, IAwsTopicBuilderWithTopicName, IAwsTopicBuilderWithQueues, IAwsTopicBuilder
{
    ...
    private string _subscribeToQueueName;
    ...
    public IAwsTopicBuilder WithSubscribedQueues(string subscribeToQueueName, params string[] queueNames)
    {
    ...
    public IAwsTopicBuilder WithQueueSettings(int? queueMaximumMessageSize)
    {
    ...
    public async Task<IConsumer> BuildConsumerAsync(int? numberOfInstances, int? receiveMaximumNumberOfMessages, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("***** Connecting {MessagingPlatform} for consumer: {Name} to Source: {MessagingSettings}", MessagingPlatform.Aws, _workerName, messagingSettings.Source);
    ...
    public async Task<IProducer> BuildProducerAsync(CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("***** Connecting {MessagingPlatform} for producer: {Name} to Source: {MessagingSettings}", MessagingPlatform.Aws, _workerName, messagingSettings.Source);
    ...
    private MessagingSettings GetMessagingSettings(string subscribeToQueueName, int? numberOfInstances = null, int? receiveMaximumNumberOfMessages = null)
    {
        ...
        return new MessagingSettings
        {
            ...
            AwsMessageVisibilityTimeoutSeconds = awsMessageVisibilityTimeoutSeconds is < AwsQueueAttributes.MessageVisibilityTimeoutMin or > AwsQueueAttributes.MessageVisibilityTimeoutMax ? 30 : awsMessageVisibilityTimeoutSeconds, // Default - 30 seconds
