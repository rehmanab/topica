...
public class PersonCreatedMessageHandlerV1(ILogger<PersonCreatedMessageHandlerV1> logger) : IHandler<PersonCreatedMessageV1>
{
    ...
    {
        logger.LogInformation("Handle: {Name} for CID: {ConversationId} for Person: {Person}", nameof(PersonCreatedMessageV1), source.ConversationId, $"{source.PersonId} : {source.PersonName}");
